...
function Services() {
  const services = servicesData.map((data, index) => {
    if (index % 2) {
      return (
  ...
  const servicesBrief = servicesData.map((data, index) => (
  ...
  return (
    <Layout>
      ...
      <div className="services-wrap">{services}</div>
      <div className="services-all">
        <div className="center-wrapper">
          <h2 className="screen-title text-center mb-70">All Our Services</h2>

          <div className="service-brief-wrap">{servicesBrief}</div>
        </div>
      </div>
